    ...
    <div class="col-5">
        ...
        <div class="row align-items-center mb-3">
            ...
            <div class="col ps-0 text-truncate">
                <a href="{{ route('profile.show', $user->id) }}" class="text-decoration-none text-dark fw-bold">{{ $user->email }}</a>
            </div>
            <div class="col-auto">
                @if ($user->id !== Auth::user()->id)
                 ...
                 <form action="{{ route('follow.destroy', $user->id) }}" method="post">
                    @csrf
                    @method('DELETE')
                    <button class="btn btn-outline-secondary fw-bold btn-sm">Fpllowing</button>
                 </form>
                 @else
                 <form action="{{ route('follow.store', $user->id) }}" method="post">
                    @csrf
                    <button class="btn btn-primay btn-sm fw-bold">Follow</button>
                 </form>
                 @endif
                 @endif
            </div>
